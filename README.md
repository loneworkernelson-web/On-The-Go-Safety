...
| **ntfy** | Push notifications for emergency alerts | Topic URL | Supports self-hosted server for greater privacy. Topics stored in Staff sheet; written on every worker POST. If trigger-path ntfy pushes are blocked by ntfy.sh shared-IP rate limiting, an optional PHP proxy script (`proxy_ntfy.php`) can be deployed to cPanel hosting to route through a clean IP — see IT Advisor Overview Section 6.5. |
